package kolok1._8_LogProcessor;
...
interface LogProcessor<T extends ILog> {
    List<T> processLogs(ArrayList<T> logs);
}
...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logs ->
                logs.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.<T>toList());

        LogProcessor<T> secondLogProcessor = logs ->
                logs.stream().filter(log -> log.getMessage().length() < 100).collect(Collectors.<T>toList());

        LogProcessor<T> thirdLogProcessor = logs ->
                logs.stream().sorted().collect(Collectors.toList());
